'use strict';(function(){Polymer({is:'og-gis-map',properties:{/**
      ...
      * min zoom directly on your tile layer.
       ...
       * @type {Array}
       */regions:{type:Array,value:function value(){return[]},observer:'_selectDefaultRegion'},hasRegions:{type:Boolean,computed:'_hasRegions(regions)',value:function value(){return[]}},regionLeft:{type:String,computed:'_calculateRegionLeft(width)'}},_hasRegions:function _hasRegions(regions){return regions&&regions.length},_focusRegion:function _focusRegion(evt){var item=evt.model.item;this.lat=item.lat;this.lng=item.lng;this.$.map.zoom=this.zoom},_selectDefaultRegion:function _selectDefaultRegion(){var _this=this;if(this.regions){this.regions.forEach(function(_region,idx){if(_region.default){_this.lat=_region.lat;_this.lng=_region.lng}})}},_calculateRegionLeft:function _calculateRegionLeft(width){if(width){var num=width.match(/\d+/)[0];return num/2+width.split(num)[1]}}})})();
