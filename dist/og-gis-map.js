'use strict';(function(){Polymer({is:'og-gis-map',behaviors:[Polymer.IronResizableBehavior],listeners:{'iron-resize':'_onIronResize'},properties:{/**
       * Component width
      ...
      * min zoom directly on your tile layer.
       ...
       *
       * @type {String}
       */tileUrl:{type:String,value:'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png'},/**
       ...
       *
       * @type {Object}
       */features:{type:Object,value:function value(){return{}}},/**
       ...
       *
       * @type {Object}
       */upstream:{type:Object,value:function value(){return{}}},/**
       * An array of objects formatted as a GeoJSON FeatureCollection with one or many Features.
       * Each feature should be a point that will be represented as a marker.
       * See the `px-map-marker-group` API documentation page for an in-depth
       * guide that explains how to configure your features.
       ...
       *
       * @type {Object}
       */midstream:{type:Object,value:function value(){return{}}},/**
       * An array of objects formatted as a GeoJSON FeatureCollection with one or many Features.
       * Each feature should be a point that will be represented as a marker.
       * See the `px-map-marker-group` API documentation page for an in-depth
       * guide that explains how to configure your features.
       ...
       *
       * @type {Object}
       */downstream:{type:Object,value:function value(){return{}}},/**
       * An array of objects formatted as a GeoJSON FeatureCollection with one or many Features.
       * Each feature should be a point that will be represented as a marker.
       * See the `px-map-marker-group` API documentation page for an in-depth
       * guide that explains how to configure your features.
       ...
       *
       * @type {Object}
       */predictive:{type:Object,value:function value(){return{}}},/**
       ...
       *
       * @type {Object}
       */colorsByType:{type:Object,value:function value(){return{}}},/**
       ...
       *
       * @type {Object}
       */clusterConfig:{type:Object,value:function value(){return{}}},contextPaneProportion:{type:Number,value:0.3},hasRegions:{type:Boolean,computed:'_hasRegions(regions)',value:function value(){return[]}},toggleMarginTop:{type:String},toggleMarginLeft:{type:String}},attached:function attached(){this.contextPaneOpen=false;//Fixes unrendered regions
var me=this;window.setTimeout(function(){var zoomIn=document.querySelector('#map a.leaflet-control-zoom-in');var zoomOut=document.querySelector('#map a.leaflet-control-zoom-out');zoomIn&&zoomIn.click();zoomOut&&zoomOut.click();me.toggleMarginLeft=document.querySelector('#map div.leaflet-control-zoom').getBoundingClientRect().left;me._adjustFilterHorizontalMargin();me._adjustMapHeight(document.querySelector('#map').getBoundingClientRect().height)},1000)},toggleContextPane:function toggleContextPane(){var currHeightNum=this.height.replace(/\D/g,'');var cpMinHeightPercentage=this.contextPaneProportion;var mapHeightPercentage=1-cpMinHeightPercentage;var mapOrigHeightPercentage=1+2*cpMinHeightPercentage;if(!this.contextPaneOpen){var newMapHeight=Math.ceil(currHeightNum*mapHeightPercentage);this.height=this.height.replace(currHeightNum,newMapHeight);this.contextPaneMinHeight=Math.ceil(currHeightNum*cpMinHeightPercentage);this.contextPaneOpen=true;this._adjustMapHeight(document.querySelector('#map').offsetHeight*mapHeightPercentage)}else{this.height=this.defaultHeight;this.contextPaneMinHeight=0;this.contextPaneMaxHeight=0;this.contextPaneOpen=false;this._adjustMapHeight(document.querySelector('#map').offsetHeight)}},invalidateSize:function invalidateSize(){this.$.map.invalidateSize()},_isValidMarkerGroup:function _isValidMarkerGroup(obj){return obj&&obj.type},_hasRegions:function _hasRegions(regions){return regions&&regions.length},_focusRegion:function _focusRegion(evt){var eventDetail=evt.detail;if(eventDetail.selected){var item=this.regions[eventDetail.key];this.lat=item.lat;this.lng=item.lng;this.$.map.zoom=item.zoom||this.zoom}},_selectDefaultRegion:function _selectDefaultRegion(){var _this=this;this.regionsDropdownData=[];if(this.regions){this.regions.forEach(function(_region,idx){var obj=_region;obj.key=idx;obj.val=_region.label;if(_region.default){_this.lat=_region.lat;_this.lng=_region.lng;obj.selected=true}_this.regionsDropdownData.push(obj)})}},_removePressed:function _removePressed(){this.upstreamPressedCls=undefined;this.midstreamPressedCls=undefined;this.downstreamPressedCls=undefined;this.predictivePressedCls=undefined},_backup:function _backup(key){if(this[key]&&this[key].type){this['_'+key]=this[key];this[key]=undefined;document.querySelector('#'+key).redraw()}},_restore:function _restore(key){var _key='_'+key;if(this[_key]&&this[_key].type){this[key]=this[_key];this[_key]=undefined;document.querySelector('#'+key).redraw()}},_hideAll:function _hideAll(){this._removePressed();this._backup('upstream');this._backup('midstream');this._backup('downstream');this._backup('predictive')},_showAll:function _showAll(){this._removePressed();this._restore('upstream');this._restore('midstream');this._restore('downstream');this._restore('predictive')},_toggleUpstreamOnly:function _toggleUpstreamOnly(){if(!this.upstreamPressedCls){this._hideAll();this.upstreamPressedCls='pressed';this._restore('upstream')}else{this._showAll()}},_toggleMidstreamOnly:function _toggleMidstreamOnly(){if(!this.midstreamPressedCls){this._hideAll();this.midstreamPressedCls='pressed';this._restore('midstream')}else{this._showAll()}},_toggleDownstreamOnly:function _toggleDownstreamOnly(){if(!this.downstreamPressedCls){this._hideAll();this.downstreamPressedCls='pressed';this._restore('downstream')}else{this._showAll()}},_togglePredictiveOnly:function _togglePredictiveOnly(){if(!this.predictivePressedCls){this._hideAll();this.predictivePressedCls='pressed';this._restore('predictive')}else{this._showAll()}},_shouldHide:function _shouldHide(bool){return bool},_adjustFilterHorizontalMargin:function _adjustFilterHorizontalMargin(newWidth,oldWidth){var comp=document.querySelector('#map');if(comp){var widthNum=comp.getBoundingClientRect().width;var toggleMarginLeft=-(widthNum*0.043);var filterMarginLeft=-(widthNum*0.0135);this.$.toggles.style.marginLeft=toggleMarginLeft+'px';this.$.filter.style.marginLeft=filterMarginLeft+'px'}this.$.map.invalidateSize()},_adjustFilterVerticalMargin:function _adjustFilterVerticalMargin(newHeight,oldHeight){if(!oldHeight){this.defaultHeight=newHeight}var comp=document.querySelector('#map');if(comp){this._adjustMapHeight(comp.getBoundingClientRect().height)}},_adjustMapHeight:function _adjustMapHeight(heightNum){if(heightNum){var toggleMarginTop=heightNum*0.25;this.$.toggles.style.marginTop=toggleMarginTop+'px'}},_compute:function _compute(contextPaneOpen){return!contextPaneOpen},_onIronResize:function _onIronResize(){console.log(arguments,this.$.map);this.$.map.invalidateSize()}})})();
